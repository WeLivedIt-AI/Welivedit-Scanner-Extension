...
function renderPanel() {
  ...
  for (const item of items) {
    ...
    const action = (item.status === "error" || isManualRetry)
      ? `<div class="welivedit-card-actions"><button data-action="retry" ${retryBusy ? "disabled aria-disabled=\"true\"" : ""}>${retryBusy ? "Retrying…" : "Retry"}</button></div>`
